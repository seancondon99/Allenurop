...
if __name__=='__main__':
    ...
    cutoffs = np.linspace(0, 1, 1001)
    totSigDet = np.zeros(len(cutoffs) - 1)
    totFPR = np.zeros(len(cutoffs) - 1)
    ...
    for key in decayTypeFrequency.keys():
        totalDecays+=decayTypeFrequency[key]


    for key in trainData.keys():
        print('beginning with '+str(key))
        ...
        if modeldir == './catBoostClassifiers/':
            ...
            for c in cdwSplit2[1:-1]:
                ...
                cdw2 += c
            cdw2 += '/'
            modeldir = cdw + 'catBoostClassifiers/samplingvariation/'+params['modelType']+'_specific'+'_NOVET'+'_'+params['weightingType']+'_'+params['randomShuffle']
        try:
            os.listdir(modeldir)
        except:
            os.mkdir(modeldir)

        zoomedModelDir = modeldir
        ...
        #model testing
        print('testing model ...')
        sigdet, fpr = testModel(model=model,modeldir=modeldir,testData=testData[key],testLabels=testLabels[key])

        #weight the sigdet and fpr
        sigdet = np.multiply(sigdet, len(testData[key].keys()))
        fpr = np.multiply(fpr, len(testData[key].keys()))
        totaltesting += len(testData[key].keys())
        totSigDet = np.add(totSigDet,sigdet)
        totFPR = np.add(totFPR,fpr)

    #make an averaged ROC plot
    totSigDet = np.divide(totSigDet,totaltesting)
    totFPR = np.divide(totFPR, totaltesting)
    plt.figure()
    plt.step(totFPR,totSigDet)
    plt.xlabel('False Positive Rate (Log KHz)')
    plt.ylabel('Signal Detection Efficiency')
    plt.title('Performance Averaged across Decay Types')
    plt.savefig(zoomedModelDir+'/averagePerformance.pdf')
